...
Show announcements run first, then artist news (Phases 1–3), then original-artist trivia (Phase 4), then historical facts (Phase 5) to fill any still-remaining slots. All phases feed the same dedup store and Buffer queue.
...
| `lp/ai.py`                                   | Claude helpers — `search_artist_news()`, `search_trivia()`, `search_historical_facts()`, `score_and_rank_topics()`, `generate_posts()`, `format_performance_context()` |
