...
def _post_tour_carousel(
    ...
    **LinkedIn gets nothing.** It used to take the lead slide, but a poster of
    ...
    drafted.
    """
    slides, acts_used = _tour_carousel_slides(artists, max_slides=max_slides,
                                              min_dates=min_dates)
    if not slides:
...
def _tour_carousel_topic(artists: list[dict], *, period: str) -> dict | None:
    """All touring acts' posters as ONE post, not one post per act.

    Client direction 2026-08-03: "we also want the tour posters to be one post,
    like, look at all these artists on tour." One act's dates is a listing; the
    whole roster's dates in a single swipe is the agency looking busy, which is
    the point of the format and the thing the Reliant post the client sent was
    doing.
    ...
    return {
        ...
        "artist":       "",
        "_act":         "",
        "sheet_key":    f"tourcarousel_{period}",
        "_image_paths": slides,
        "_visual":      True,
...
def _draft_visual_posts(
    *,
    ...
    Two kinds, in order: the tour carousel (**one** post covering every touring
    act, per client direction 2026-08-03) and then one clip per act. So a floor
    of 2 comes back as the carousel plus a clip. Acts already scheduled this run
    are skipped, and each type carries a period-stamped dedup key: the carousel
    once a week, a clip once a quarter per act.
    ...
    # The tour carousel is ONE post covering every touring act, so it is a single
    # candidate rather than one per act.
    candidates: list[dict] = []
    if f"tourcarousel_{week}" not in used:
        if (carousel := _tour_carousel_topic(artists, period=week)):
            candidates.append(carousel)

    # Clips are per act and cost an ffmpeg run over the network, so stop as soon
    # as there are enough to reach the floor.
    for artist in artists:
        ...
        if not name or name in taken:
            continue
        if f"clip_{_slugify(name)}_{quarter}" in used:
            continue
    ...
    for topic in candidates:
        ...
        media = topic.get("_video_path") or topic.get("_image_paths") or []
        if isinstance(media, str):
            media = [media]
        urls = media if dry_run else [u for m in media if (u := upload_media(m))]
        if not urls:
            log.warning("Visual: could not host %s, skipping", topic.get("headline", ""))
            continue
        booking = config.STEVE_CALENDAR_LINK or config.LP_HOMEPAGE
        ...
        for platform in _platforms_for(topic["hook_type"]):
            ...
            if post_draft_to_buffer(
                text, profile_id, platform=platform, dry_run=dry_run,
                scheduled_at=slot,
                video=urls[0] if is_video else None,
                images=None if is_video else urls,
            ):
                posted = True
                log.info(
                    "  %s %s (%d) %s",
                    platform,
                    "clip" if is_video else "tour carousel",
                    len(urls),
    ...
    if drafted < want:
        log.warning(
            "Visual posts: wanted %d, drafted %d. The tour carousel has already run "
            "this week, or no further act has usable Vimeo footage.",
            want, drafted,
        )
    return drafted
