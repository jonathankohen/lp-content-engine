# Audience: Buyers

## Who They Are

Talent buyers, venue bookers, event promoters, and other booking agencies.
They have budgets, calendars to fill, and reputations on the line. They need
acts that reliably draw crowds, deliver professional performances, and make
patrons happy enough to come back.

## What They Want

- Proof that our acts sell tickets. Real numbers, sold-out shows,
  repeat engagements.
- Patron satisfaction evidence: reviews, crowd photos, rave testimonials.
- Confidence that booking our acts is a safe, low-risk decision.
- A sense of what the act actually looks like live: energy, production value,
  audience reaction.

## How to Talk to Them

- Lead with results: "sold out in 3 days," "standing ovation," "venue re-booked
  for next season."
- Social proof is everything. Photos of packed rooms. Quotes from happy patrons.
  Reviews from other buyers.
- Position Love Productions as a trusted partner, not just a vendor. We know
  our acts, we know live music, we make buyers look good.
- Make booking feel obvious. If they're on the fence, the content should tip them.

## How NOT to talk to them

They work in live entertainment. Everything below reads as talking down to them,
and all of it appeared in the post the client rejected (see
[Reference Posts](../voice/reference-posts.md)):

- Explaining what makes a good act, or why audiences enjoy live music.
- Telling them what a booking will do for their venue. They know.
- Claims shaped like data with no number behind them ("fills seats before the
  first ad runs").
- Any paragraph they have to read to reach the point.

## Content That Converts Buyers

These are the types this engine actually produces. Do not reach for a format
that is not on this list.

- Re-booking announcements: a venue that booked the act again, with the count.
  The strongest proof we have.
- Published testimonials from a venue, buyer, or review, quoted verbatim.
- Press coverage, awards, or notable milestones for our acts.
- Act spotlights built on one hard credential.
- Agency credibility, from [Agency Facts](../engine/agency-facts.md) only.
- Tour-date cards and carousels: a wall of real dates is itself the proof.

**Post-show recaps are deliberately not produced** (client direction
2026-07-28). Without real attendance or ticket-sales numbers to lead with they
are just another show-shaped post. Do not write one.
